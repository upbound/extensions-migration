...
func FileSystemResource(mg resource.Managed) ([]resource.Managed, error) {
	source := mg.(*srcv1alpha1.FileSystem)
	target := &targetv1beta1.FileSystem{}
	if _, err := migration.CopyInto(source, target, targetv1beta1.FileSystem_GroupVersionKind, "spec.forProvider.tags"); err != nil {
	...
	target.Spec.ForProvider.KMSKeyID = source.Spec.ForProvider.KMSKeyID
	return []resource.Managed{
		target,
	}, nil
}

func FileSystemComposition(sourceTemplate v1.ComposedTemplate, convertedTemplates ...*v1.ComposedTemplate) error {
